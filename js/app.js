(() => {
    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            ...
            if (bodyLockStatus && e.target.closest(".menu-header__close")) functions_menuClose();
        }));
    }
    function menuOpen() {
        bodyLock();
    ...
    function documentActions(e) {
        const el = e.target;
        if (productCart && el.closest(".nav-page__cart") && !el.closest(".nav-page__products")) productCart.classList.toggle("nav-page__cart_active"); else if (productCart && !el.closest(".nav-page__products")) productCart.classList.remove("nav-page__cart_active");
        ...
        if (el.closest(".card-product__image")) if (el.closest(".card-product__image_main")) return false; else {
    ...
    function updateCart(itemId, productButton, inputValue, productAdd = true) {
        fetch("../dilivery.html").then((response => response.text())).then((data => {
            ...
            const cartProduct = document.querySelector(`[data-cart-id='${itemId}']`);
            ...
            if (productAdd) {
                if (cartQuantity) if (inputValue) cartQuantity.innerHTML = +cartQuantity.innerHTML + +inputValue; else cartQuantity.innerHTML = ++cartQuantity.innerHTML; else cartIcon.insertAdjacentHTML("beforeend", `<span>${quantityOfProducts}</span>`);
                if (!cartProduct) {
                    const product = htmlDocument.querySelector(`[data-id="${itemId}"]`);
                    console.log(itemId);
                    const cartProductImage = product.querySelector(".tabs-meal__image").innerHTML;
                    const cartProductName = product.querySelector(".tabs-meal__name").innerHTML;
                    const cartProductPrice = product.querySelector(".tabs-meal__price").innerHTML;
                    ...
                    cartlist.insertAdjacentHTML("beforeend", `<div data-cart-id='${itemId}' class="nav-page__item">${cartProductContent}</div>`);
                } else {
                    const cartProductQuantity = cartProduct.querySelector(".nav-page__quantity > span");
                    if (inputValue) cartProductQuantity.innerHTML = +cartProductQuantity.innerHTML + +inputValue; else cartProductQuantity.innerHTML = ++cartProductQuantity.innerHTML;
                }
